...
|---|------|--------|-------------|----------------------------|
| 10.1 | **MCP (Model Context Protocol) native gateway** | Large | First-class support for AI agent traffic: route MCP tool calls, apply per-agent rate limiting, track token usage per agent, and provide agent-specific API key scoping. The 2026 gateway trend — but ServGate has WASM + AI middleware already. | AI-native from day one. Competitors bolt MCP on as afterthought. |
| 10.2 | **Compiler-aware route registration** | Small | Serv-lang services self-announce routes at startup (`servgate://` connector). No manual route config, no service mesh sidecar. The gateway knows every endpoint the compiler produced. | Tight compiler→gateway integration. Impossible when gateway and language are separate products. |
| 10.3 | **WASM middleware hot-swap without request drops** | Medium | Upload new WASM middleware while serving live traffic — existing in-flight requests finish with old logic, new requests use new logic. Zero-downtime middleware deploys. | WASM module isolation + request-level binding. Envoy needs full pod restart for filter changes. |
...
| 10.5 | **Cost-aware routing for LLM backends** | Medium | Route AI requests to the cheapest model that meets quality SLA: try GPT-4o-mini first, escalate to GPT-4 only if confidence is low. Per-route cost budget enforcement. | Purpose-built for AI backend routing. Generic gateways have no concept of model cost/quality tradeoff. |
...
| 10.9 | **Per-route WASM A/B testing** | Medium | Attach two WASM middleware versions to a route with traffic split. Compare latency/error rates between them in ServConsole. Test middleware changes in production with controlled rollout. | WASM sandboxing enables safe side-by-side execution of untrusted code in the same process. |
